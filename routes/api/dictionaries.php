...
Route::prefix('/api/dictionaries')->middleware(['auth:sanctum'])->group(function () {
    ...
    Route::patch('/{alias}', [DictionaryController::class, 'sync'])->middleware([PositionType::middleware(PositionType::admin, PositionType::staff), Permission::middleware(Permission::system__dictionaries)]);
    Route::get('/{alias}/item/{ID?}', [DictionaryController::class, 'get']);
    Route::put('/{alias}/item/{ID?}', [DictionaryController::class, 'update']);
